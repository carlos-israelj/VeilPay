...
// Always override in production (Render) to ensure HTTPS
if (process.env.RENDER) {
  // Running on Render - force HTTPS
  process.env.X402_BASE_URL = 'https://veilpay-x402-relayer.onrender.com';
  console.log('[x402] Render environment detected - using HTTPS:', process.env.X402_BASE_URL);
} else if (!process.env.X402_BASE_URL) {
  // Local development - default to HTTP
  const port = process.env.PORT || 4000;
  process.env.X402_BASE_URL = `http://localhost:${port}`;
  console.log('[x402] Local development - using HTTP:', process.env.X402_BASE_URL);
}
